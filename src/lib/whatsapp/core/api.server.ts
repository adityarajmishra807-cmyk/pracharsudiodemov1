...
import type { ApiEnvelope, WhatsAppChat, WhatsAppHealth, WhatsAppMessage, WhatsAppMessageInput, WhatsAppSession } from "./types";
...
class WhatsAppApiError extends Error { status: number; details?: unknown; constructor(message: string, status = 500, details?: unknown) { super(message); this.name = "WhatsAppApiError"; this.status = status; this.details = details; } }
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  try {
    const response = await fetch(`${root}${path}`, { ...init, headers: { "content-type": "application/json", ...(backendKey() ? { authorization: `Bearer ${backendKey()}` } : {}), ...(init?.headers || {}) }, cache: "no-store", signal: AbortSignal.timeout(15000) });
...
export const startWhatsAppSession = createServerFn({ method: "POST" }).validator((value: unknown) => z.object({ sessionId: SessionId }).parse(value)).handler(({ data }) => request<{ success: boolean; message?: string }>(`/api/sessions/${encodeURIComponent(data.sessionId)}/start`, { method: "POST" }));
export const getWhatsAppSessionStatus = createServerFn({ method: "POST" }).validator((value: unknown) => z.object({ sessionId: SessionId }).parse(value)).handler(async ({ data }) => (await request<ApiEnvelope<WhatsAppSession>>(`/api/sessions/${encodeURIComponent(data.sessionId)}/status`)).data);
export const logoutWhatsAppSession = createServerFn({ method: "POST" }).validator((value: unknown) => z.object({ sessionId: SessionId }).parse(value)).handler(({ data }) => request<{ success: boolean; message?: string }>(`/api/sessions/${encodeURIComponent(data.sessionId)}/logout`, { method: "POST" }));
export const deleteWhatsAppSession = createServerFn({ method: "POST" }).validator((value: unknown) => z.object({ sessionId: SessionId }).parse(value)).handler(({ data }) => request<{ success: boolean; message?: string }>(`/api/sessions/${encodeURIComponent(data.sessionId)}`, { method: "DELETE" }));
