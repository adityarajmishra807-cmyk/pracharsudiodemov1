...
const NAV: NavItem[] = [
  { to: "/dashboard", label: "Dashboard", icon: LayoutDashboard, permission: "dashboard", primaryMobile: true },
  { to: "/leads", label: "CRM / Leads", icon: UserRound, permission: "leadsView", primaryMobile: true },
  { to: "/inbox", label: "WhatsApp Inbox", icon: MessageSquare, permission: ["inboxReply", "inboxAssign"], primaryMobile: true },
  { to: "/templates", label: "Templates", icon: FileText, permission: ["templatesUse", "templatesManage"], primaryMobile: true },
  { to: "/whatsapp", label: "WhatsApp Manager", icon: MessageCircle, permission: "inboxReply" },
  ...
  return (
    <nav className="flex flex-col gap-1" aria-label="Main">
      {items.map(({ to, label, icon: Icon }) => (
        <Link key={to} to={to} onClick={onNavigate} className="flex min-h-11 items-center gap-3 rounded-md px-3 text-sm font-medium text-white/75 transition-colors hover:bg-white/10 hover:text-white data-[status=active]:bg-primary data-[status=active]:text-primary-foreground" activeProps={{ "data-status": "active" }}>
  ...
  return (
    <div className="border-t border-white/10 pt-3">
      <div className="flex items-center gap-3 px-3 py-2">
        <span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-white/10 text-sm font-semibold text-white">{(isOwner ? state.settings.ownerName || "O" : currentMember?.name || "M").slice(0, 1).toUpperCase()}</span>
        <div className="min-w-0">
          <p className="truncate text-sm font-semibold text-white">{isOwner ? state.settings.ownerName || "Workspace owner" : currentMember?.name}</p>
          <p className="truncate text-xs text-white/60">{isOwner ? "Owner" : "Member"}</p>
        </div>
      </div>
      <button type="button" onClick={() => { onNavigate?.(); signOut(); void router.navigate({ to: "/" }); }} className="mt-1 flex min-h-11 w-full items-center gap-3 rounded-md px-3 text-sm font-medium text-white/75 transition-colors hover:bg-white/10 hover:text-white">
  ...
  return (
    <div className="min-h-screen bg-surface">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col bg-navy px-3 py-4 lg:flex">
        <div className="px-2 pb-4"><Logo onDark /></div>
          ...
          <div className="flex min-w-0 items-center gap-3">
            <Sheet open={menuOpen} onOpenChange={setMenuOpen}>
              ...
              <SheetContent side="left" className="w-[17rem] border-none bg-navy p-3">
                ...
                <div className="mt-4"><AccountBlock onNavigate={() => setMenuOpen(false)} /></div>
              </SheetContent>
            </Sheet>
      ...
      <nav className="safe-bottom fixed inset-x-0 bottom-0 z-30 flex items-stretch border-t border-border bg-white pt-1.5 lg:hidden" aria-label="Primary">
        {primary.map(({ to, label, icon: Icon }) => {
          ...
          return <Link key={to} to={to} className={cn("flex min-h-11 flex-1 flex-col items-center justify-center gap-1 px-1 text-[11px] font-medium", active ? "text-primary" : "text-muted-foreground")}><Icon className="size-5" aria-hidden="true" /><span className="max-w-full truncate">{label.split(" ")[0]}</span></Link>;
        })}
        {overflow.length > 0 ? <button type="button" onClick={() => setMenuOpen(true)} className="flex min-h-11 flex-1 flex-col items-center justify-center gap-1 px-1 text-[11px] font-medium text-muted-foreground"><MoreHorizontal className="size-5" aria-hidden="true" />More</button> : null}
      </nav>
