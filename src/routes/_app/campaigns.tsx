...
export const Route = createFileRoute("/_app/campaigns")({
  head: () => ({
    meta: [
      { title: "Campaigns — Prachar Studio" },
      { name: "description", content: "Persistent WhatsApp campaign manager." },
    ],
  }),
...
type CampaignType =
  "text" | "media" | "media-text" | "buttons" | "list" | "media-buttons" | "media-list";
type ButtonDraft = {
  id: string;
  displayText: string;
  type: "quickReply" | "url" | "phone" | "copy";
  url?: string;
  phoneNumber?: string;
  copyCode?: string;
};
...
function CampaignsPage() {
  ...
  const [queueMonitor, setQueueMonitor] = useState<Awaited<
    ReturnType<typeof campaignsApi.queue>
  > | null>(null);
  ...
  const load = async () => {
    try {
      const [items, available, savedAudiences, savedTemplates, queue] = await Promise.all([
        campaignsApi.list(),
        sessionsApi.list(),
        audiencesApi.list(),
        templatesApi.list(),
      ...
      toast.error(e instanceof Error ? e.message : "Could not load campaign data");
    }
  };
  useEffect(() => {
    void load();
  ...
  const resetComposer = () => {
    ...
    setTemplateId("");
    setType("text");
    setText("Hello {{name}}, we have an offer for {{company}}.");
    setCaption("");
    setMedia(null);
    setTitle("");
    setDescription("");
  ...
  const updateRow = (sectionIndex: number, rowIndex: number, patch: Partial<RowDraft>) =>
    setSections((items) =>
      items.map((s, i) =>
        i === sectionIndex
          ? { ...s, rows: s.rows.map((r, j) => (j === rowIndex ? { ...r, ...patch } : r)) }
          : s,
      ),
    );
  ...
  const selectAudience = async (id: string) => {
    ...
    try {
      ...
      if (audience.total > 250) {
        setRecipientRows([]);
        toast.error(
          `This audience contains ${audience.total} recipients. Campaigns currently support up to 250.`,
        );
        return;
      }
  ...
  const selectTemplate = async (id: string) => {
    ...
    try {
      ...
      if (
        ["media", "media-buttons", "media-list"].includes(nextType) &&
        data.media &&
      ...
      if (["buttons", "media-buttons"].includes(nextType)) {
        ...
        const parsed = Array.isArray(data.buttons)
          ? data.buttons
          : (() => {
              try {
                return JSON.parse(String(data.buttonsJson || "[]"));
              } catch {
                return [];
              }
            })();
        if (Array.isArray(parsed) && parsed.length)
          setButtons(
            parsed.map((b: Record<string, unknown>) => ({
              id: String(b.id || ""),
              displayText: String(b.displayText || ""),
              type: b.url ? "url" : b.phoneNumber ? "phone" : b.copyCode ? "copy" : "quickReply",
              ...(b.url ? { url: String(b.url) } : {}),
              ...(b.phoneNumber ? { phoneNumber: String(b.phoneNumber) } : {}),
              ...(b.copyCode ? { copyCode: String(b.copyCode) } : {}),
            })),
          );
      }
      if (["list", "media-list"].includes(nextType)) {
        ...
        const parsed = Array.isArray(data.sections)
          ...
          : (() => {
              try {
                return JSON.parse(String(data.sectionsJson || "[]"));
              } catch {
                return [];
              }
            })();
          ...
          setSections(
            parsed.map((s: Record<string, unknown>) => ({
              ...
              rows: Array.isArray(s.rows)
                ? s.rows.map((row: Record<string, unknown>) => ({
  ...
  const create = async () => {
    ...
    if ((type === "text" || type === "media-text") && !text.trim() && !caption.trim())
    ...
    if (
      ...
      (!title.trim() ||
        !buttons.length ||
        buttons.length > 3 ||
        buttons.some((b) => !b.id.trim() || !b.displayText.trim()))
    )
      return toast.error("Add a title and 1–3 valid buttons.");
    ...
    if (
      ...
      (!title.trim() ||
        ...
        !rowCount ||
        rowCount > 10 ||
        sections.some((s) => s.rows.some((r) => !r.rowId.trim() || !r.title.trim())))
    )
      return toast.error("Add a list title, menu button and 1–10 valid rows.");
    setBusy(true);
    try {
      ...
      if (type === "media-text") {
        payload.media = media;
        payload.caption = caption.trim() || text.trim();
      }
      if (type === "media") {
        payload.media = media;
      ...
      if (requiresButtons) {
        ...
        payload.buttons = buttons.map(({ type: buttonType, ...b }) => ({
          ...b,
          ...(buttonType === "url" ? { url: b.url } : {}),
          ...(buttonType === "phone" ? { phoneNumber: b.phoneNumber } : {}),
          ...(buttonType === "copy" ? { copyCode: b.copyCode } : {}),
        }));
      }
      if (requiresList) {
        payload.title = title.trim();
        payload.description = description.trim();
        payload.footerText = footer.trim();
      ...
      toast.error(e instanceof Error ? e.message : "Campaign creation failed");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Campaigns"
        description="Persistent WhatsApp campaigns backed by the Evolution worker."
        actions={
      ...
      {queueMonitor && (
        <section className="rounded-lg border border-border bg-card p-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="font-semibold">Campaign queue</p>
              <p className="text-xs text-muted-foreground">
                {queueMonitor.workerRunning ? "Worker active" : "Worker idle"} ·{" "}
                {queueMonitor.maxWorkers} worker slot{queueMonitor.maxWorkers === 1 ? "" : "s"}
              </p>
            </div>
            <div className="flex flex-wrap gap-2 text-xs">
              <span className="rounded-full border px-2.5 py-1">Queued {queueMonitor.queued}</span>
              <span className="rounded-full border px-2.5 py-1">
                Running {queueMonitor.running}
              </span>
          ...
          {queueMonitor.items.length > 0 && (
            <div className="mt-3 overflow-x-auto rounded-md border">
              <table className="w-full min-w-[620px] text-sm">
                ...
                <tbody>
                  {queueMonitor.items.map((item) => (
                    <tr key={item.campaign_id} className="border-b last:border-0">
                      <td className="p-2.5 font-medium">{item.name}</td>
                      <td className="p-2.5">
                        <StatusBadge value={item.status} />
                      </td>
                      <td className="p-2.5">{item.attempts}</td>
                      <td className="p-2.5 text-xs text-muted-foreground">
                        {formatDate(item.updated_at)}
                      </td>
                      <td className="max-w-xs truncate p-2.5 text-xs text-destructive">
                        {item.error || "—"}
                      </td>
                    </tr>
                  ))}
      ...
      {!campaigns.length ? (
        <EmptyState
          ...
          description="Create a persistent campaign and the background worker will process it."
            ...
            <li key={c.id} className="rounded-lg border border-border bg-card p-4">
              <div className="flex justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {c.type} · {c.instance} · {formatDate(c.createdAt)}
                  </p>
                </div>
              ...
              <div className="mt-3 grid grid-cols-4 gap-2 text-sm">
                ...
                <div>
                  ...
                  <p>{Math.max(0, c.total - c.sent - c.failed)}</p>
                </div>
              </div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {["queued", "running"].includes(c.status) && (
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => void action(c.id, campaignsApi.pause, "Paused")}
                  >
                ...
                {c.status === "paused" && (
                  <Button
                    size="sm"
                    onClick={() => void action(c.id, campaignsApi.resume, "Resumed")}
                  >
                ...
                {["queued", "running", "paused"].includes(c.status) && (
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => void action(c.id, campaignsApi.cancel, "Cancelled")}
                  >
                ...
                {c.failed > 0 && (
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => void action(c.id, campaignsApi.retryFailed, "Retry queued")}
                  >
                    <RotateCcw className="size-3.5" /> Retry failed
                  </Button>
                )}
                <Button size="sm" variant="outline" onClick={() => void showDetails(c.id)}>
                  Delivery details
                </Button>
      ...
      <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}>
        <DialogContent className="max-h-[92svh] overflow-y-auto sm:max-w-4xl">
          <DialogHeader>
            <DialogTitle>
              Delivery details{selectedCampaign ? ` — ${selectedCampaign.name}` : ""}
            </DialogTitle>
            <DialogDescription>
              Recipient-level send and Evolution delivery status tracking.
            </DialogDescription>
          </DialogHeader>
          {detailsBusy || !selectedCampaign ? (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
                {[
                  ...
                  [
                    ...
                    Math.max(
                      0,
                      selectedCampaign.total - selectedCampaign.sent - selectedCampaign.failed,
                    ),
                  ],
              ...
              <div className="overflow-x-auto rounded-md border">
                <table className="w-full min-w-[720px] text-sm">
                  <thead>
                    <tr className="border-b bg-muted/30 text-left text-xs uppercase text-muted-foreground">
                      ...
                      <th className="p-3">Evolution message</th>
                  ...
                  <tbody>
                    {selectedCampaign.recipients.map((recipient, index) => {
                      ...
                      const latest = statuses.length
                        ? (statuses[statuses.length - 1] as Record<string, unknown>)
                        : null;
                      ...
                      const updatedAt = latest?.updatedAt
                        ? formatDate(String(latest.updatedAt))
                        : result?.timestamp
                      ...
                      return (
                        <tr key={index} className="border-b last:border-0">
                          ...
                          <td className="p-3">
                            <code className="text-xs">{messageId}</code>
                          </td>
                          <td className="p-3">
                ...
                {!selectedCampaign.recipients.length && (
                  <p className="p-6 text-center text-sm text-muted-foreground">No recipients.</p>
                )}
              </div>
              <p className="text-xs text-muted-foreground">
                Evolution statuses tracked: ERROR, PENDING, SERVER_ACK, DELIVERY_ACK, READ, DELETED
                and PLAYED.
              </p>
            </div>
          )}
          <DialogFooter>
            ...
            {selectedCampaign && (
              <Button onClick={() => void showDetails(selectedCampaign.id)} disabled={detailsBusy}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[92svh] overflow-y-auto sm:max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Build the exact Evolution message payload and queue it for background delivery.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <Select value={instance} onValueChange={setInstance}>
                  ...
                  <SelectContent>
                    {connected.map((s) => (
                      <SelectItem key={s.instanceName} value={s.instanceName || ""}>
                        {s.instanceName}
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => void selectTemplate(value === "__none__" ? "" : value)}
              >
                <SelectTrigger>
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-md border p-3 space-y-3">
                <div>
                  ...
                  <Select
                    value={audienceId || "__manual__"}
                    onValueChange={(value) =>
                      void selectAudience(value === "__manual__" ? "" : value)
                    }
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Import recipients manually" />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  {audienceBusy && (
                    <p className="mt-1 text-xs text-muted-foreground">Loading audience…</p>
                  )}
                  {audienceId && !audienceBusy && (
                    <p className="mt-1 text-xs text-muted-foreground">
                      Recipients are snapshotted into this campaign when it is queued.
                    </p>
                  )}
                </div>
                {!audienceId && (
                  <RecipientImporter value={recipientRows} onChange={setRecipientRows} max={250} />
                )}
                {audienceId && !!recipientRows.length && (
                  <p className="text-sm text-muted-foreground">
                    {recipientRows.length} recipients loaded from the selected audience.
                  </p>
                )}
              </div>
            </div>
            ...
            {requiresMedia && (
              <div className="space-y-2">
                <Label>Media (maximum 8 MB)</Label>
                <Input
                  type="file"
                  ...
                  onChange={async (e) => {
                    ...
                    if (file.size > 8 * 1024 * 1024) {
                      e.target.value = "";
                      return toast.error("Media must be 8 MB or smaller.");
                    }
                    try {
                      ...
                      toast.error(error instanceof Error ? error.message : "Could not read media.");
                    }
                  }}
                />
                <p className="text-xs text-muted-foreground">
                  {media ? `Selected: ${media.fileName}` : "Choose an image, video or document."}
                </p>
              </div>
            )}

            {["media", "media-text"].includes(type) && (
              <div>
                <Label>{type === "media-text" ? "Caption" : "Optional caption"}</Label>
                <Textarea
                  value={caption}
            ...
            {requiresButtons && (
              <div className="space-y-3 rounded-md border p-3">
                ...
                {buttons.map((b, i) => (
                  <div key={i} className="grid gap-2 rounded border p-2 sm:grid-cols-5">
                    <Input
                      placeholder="ID"
                      value={b.id}
                    ...
                    <Button
                      ...
                      variant="ghost"
                      size="icon"
                      onClick={() => setButtons((items) => items.filter((_, j) => j !== i))}
                    >
                ...
                <Button
                  ...
                  onClick={() => setButtons((items) => [...items, emptyButton()])}
                >
                  Add button
                </Button>
              </div>
            ...
            {requiresList && (
              <div className="space-y-3 rounded-md border p-3">
                ...
                {sections.map((s, si) => (
                  <div key={si} className="space-y-2 rounded border p-2">
                    <div className="flex gap-2">
                      <Input
                        ...
                        onChange={(e) =>
                          setSections((items) =>
                            items.map((x, i) => (i === si ? { ...x, title: e.target.value } : x)),
                          )
                        }
                      />
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        onClick={() => setSections((items) => items.filter((_, i) => i !== si))}
                      >
                        <Trash2 className="size-4" />
                      </Button>
                    </div>
                    {s.rows.map((r, ri) => (
                      <div key={ri} className="grid gap-2 sm:grid-cols-4">
                        <Input
                          placeholder="Row ID"
                          value={r.rowId}
                        ...
                        <Button
                          ...
                          onClick={() =>
                            setSections((items) =>
                              items.map((x, i) =>
                                i === si ? { ...x, rows: x.rows.filter((_, j) => j !== ri) } : x,
                              ),
                            )
                          }
                        >
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setSections((items) =>
                          items.map((x, i) =>
                            i === si ? { ...x, rows: [...x.rows, emptyRow()] } : x,
                          ),
                        )
                      }
                      disabled={sections.reduce((n, x) => n + x.rows.length, 0) >= 10}
                    >
                      Add row
                    </Button>
                  </div>
                ...
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => setSections((items) => [...items, emptySection()])}
                >
                  Add section
                </Button>
              </div>
            ...
            <div>
              <Label>Delay between recipients (ms)</Label>
              <Input
                type="number"
                min={1500}
                max={10000}
              ...
              />
              <p className="mt-1 text-xs text-muted-foreground">Safety minimum: 1500 ms.</p>
            </div>
          </div>
          <DialogFooter>
