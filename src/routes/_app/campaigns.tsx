...
function CampaignsPage() {
  ...
  const connected =
    state.whatsappSessions?.filter?.((s: any) =>
  ...
  const create = async () => {
    const rows = recipients
      ...
      .filter(Boolean)
      .map((phone) => ({ phone }));
    if (!name.trim() || !instance || !rows.length || !text.trim())
      return toast.error("Campaign name, instance, message and recipients are required.");
  ...
  const action = async (id: string, fn: (id: string) => Promise<Campaign>, msg: string) => {
    try {
      await fn(id);
      toast.success(msg);
      await load();
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-h-[92svh] overflow-y-auto sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Create a persistent queued campaign.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select value={type} onValueChange={setType}>
                ...
                <SelectContent>
                  {["text", "media-text", "buttons", "list", "media-buttons", "media-list"].map(
                    (v) => (
            ...
            <div>
              <Label>Delay (ms, minimum 1500)</Label>
              <Input
                type="number"
                min={1500}
                max={10000}
