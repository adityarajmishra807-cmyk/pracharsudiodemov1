...
function WhatsAppManagerPage() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (active) setAccounts((current) => current.map((account) => account.id === selected.id ? { ...account, status: "ERROR" } : account));
      }
  ...
  const addAccount = async () => {
    ...
    try {
      ...
      setQr(result.qr);
      setAccounts((current) => current.map((item) => item.id === account.id ? { ...item, status: result.status as SessionStatus } : item));
    } catch (error) {
  ...
  const reconnect = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Could not reconnect");
    }
  };
