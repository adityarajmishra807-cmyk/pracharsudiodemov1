...
export function useSessions() {
  return useQuery({ queryKey: sessionsQueryKey, queryFn: sessionsApi.list, refetchInterval: 8_000 });
}
...
export function useSessionMutation() {
  ...
  return {
    ...
    sendText: useMutation({ mutationFn: ({ instance, number, text }: { instance: string; number: string; text: string }) => sessionsApi.sendText(instance, number, text) }),
  };
}
