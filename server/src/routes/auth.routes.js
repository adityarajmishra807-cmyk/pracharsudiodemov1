...
const legacyHash=(v)=>crypto.createHash('sha256').update(String(v)).digest('hex');
const makeToken=()=>crypto.randomBytes(32).toString('hex');
const defaults={dashboard:true,leadsView:true,leadsEdit:false,inboxReply:false,inboxAssign:false,templatesUse:true,templatesManage:false,campaigns:false,automations:false,analytics:false,settings:false};
...
export async function hashPassword(password){
  ...
  return SCRYPT_PREFIX+salt.toString('base64')+'$'+Buffer.from(derived).toString('base64');
}
export async function verifyPassword(password,stored){
  if(String(stored||'').startsWith(SCRYPT_PREFIX)){
    const [,salt64,hash64]=String(stored).split('$');
    if(!salt64||!hash64)return false;
    const derived=await scrypt(String(password),Buffer.from(salt64,'base64'),64,{N:16384,r:8,p:1});
  ...
  return legacyHash(password)===String(stored||'');
}
export async function authUser(req,res,next){
  try{
    ...
    const r=await pool.query(`SELECT u.id,u.email,u.role,u.status,u.member_id,m.name,m.permissions,u.password_hash
      FROM auth_sessions s JOIN app_users u ON u.id=s.user_id LEFT JOIN members m ON m.id=u.member_id
      WHERE s.token_hash=$1 AND s.expires_at>NOW() LIMIT 1`,[legacyHash(raw)]);
...
authRouter.post('/setup',async(req,res,next)=>{
 try{
   ...
   const id=makeId('user'),token=makeToken();
   await pool.query('INSERT INTO app_users(id,email,password_hash,role) VALUES($1,$2,$3,$4)',[id,email,await hashPassword(password),'owner']);
   ...
   await pool.query(`INSERT INTO auth_sessions(id,user_id,token_hash,expires_at) VALUES($1,$2,$3,NOW()+INTERVAL '30 days')`,[makeId('sess'),id,legacyHash(token)]);
...
authRouter.post('/login',async(req,res,next)=>{
 try{
  const email=String(req.body?.email||'').trim().toLowerCase(),password=String(req.body?.password||'');
  const r=await pool.query(`SELECT u.id,u.email,u.role,u.status,u.member_id,m.name,m.permissions,u.password_hash FROM app_users u
    LEFT JOIN members m ON m.id=u.member_id WHERE lower(u.email)=lower($1) LIMIT 1`,[email]);
  if(!r.rows[0]||r.rows[0].status!=='active'||!(await verifyPassword(password,r.rows[0].password_hash)))return res.status(401).json({ok:false,message:'Invalid email or password.'});
  const u=r.rows[0],token=makeToken();
  if(!String(u.password_hash||'').startsWith(SCRYPT_PREFIX)) await pool.query('UPDATE app_users SET password_hash=$2 WHERE id=$1',[u.id,await hashPassword(password)]);
  await pool.query(`INSERT INTO auth_sessions(id,user_id,token_hash,expires_at) VALUES($1,$2,$3,NOW()+INTERVAL '30 days')`,[makeId('sess'),u.id,legacyHash(token)]);
...
authRouter.post('/logout',authUser,async(req,res,next)=>{
 try{const raw=String(req.headers.authorization||'').replace(/^Bearer\s+/i,'').trim();await pool.query('DELETE FROM auth_sessions WHERE token_hash=$1',[legacyHash(raw)]);res.status(204).end();}catch(e){next(e);}
});
