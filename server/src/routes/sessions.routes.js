...
sessionsRouter.get('/', async (_req, res, next) => {
  try { res.json(await listInstances()); } catch (error) { next(error); }
});
...
sessionsRouter.post('/', async (req, res, next) => {
  try {
    ...
    if (!config.instanceNamePattern.test(instanceName)) {
      return res.status(400).json({ ok: false, message: 'Instance name must contain only lowercase letters, numbers, _ or -, and be 1-64 characters.' });
    }
...
sessionsRouter.get('/:instance/connect', async (req, res, next) => {
  try { res.json(await connectInstance(instance(req))); } catch (error) { next(error); }
});
...
sessionsRouter.post('/:instance/restart', async (req, res, next) => {
  try { res.json(await restartInstance(instance(req))); } catch (error) { next(error); }
});

sessionsRouter.post('/:instance/disconnect', async (req, res, next) => {
  try { res.json(await logoutInstance(instance(req))); } catch (error) { next(error); }
});
...
sessionsRouter.post('/:instance/send-text', async (req, res, next) => {
  try {
    ...
    res.json(await sendText(instance(req), number, text, { linkPreview: Boolean(req.body?.linkPreview), delayMs: req.body?.delayMs }));
  } catch (error) { next(error); }
});

sessionsRouter.post('/:instance/send-media', async (req, res, next) => {
  try {
    const number = String(req.body?.number || '').trim();
...
sessionsRouter.delete('/:instance', async (req, res, next) => {
  try { res.json(await deleteInstance(instance(req))); } catch (error) { next(error); }
});
