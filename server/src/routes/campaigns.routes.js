...
function campaignDelay(body) {
  ...
  return Math.max(
    MIN_DELAY_MS,
    Math.min(Number.isFinite(requested) ? requested : MIN_DELAY_MS, 10000),
  );
}
function validateRecipients(recipients) {
  if (!Array.isArray(recipients) || !recipients.length)
    throw Object.assign(new Error("At least one valid recipient is required."), { status: 400 });
  if (recipients.length > MAX_RECIPIENTS)
    throw Object.assign(
      new Error(`A single campaign is limited to ${MAX_RECIPIENTS} recipients in this phase.`),
      { status: 400 },
    );
}
...
async function runCampaign(recipients, delayMs, sender) {
  ...
  for (let index = 0; index < recipients.length; index += 1) {
    ...
    if (!number || number.length < 8 || number.length > 15)
      results.push({
        index,
        phone: recipient.phone || recipient.number || "",
        ok: false,
        message: "Invalid phone number.",
      });
    else {
      try {
        await sender(number, recipient);
...
campaignsRouter.post("/text", async (req, res, next) => {
  try {
    ...
    return resultResponse(
      res,
      await runCampaign(recipients, campaignDelay(req.body), (number, recipient) =>
        sendText(instance, number, personalize(text, recipient), { delayMs: 0 }),
      ),
    );
  } catch (error) {
    next(error);
...
campaignsRouter.post("/media", async (req, res, next) => {
  try {
    ...
    if (!["image", "video", "document"].includes(mediatype))
      return res
        .status(400)
        .json({ ok: false, message: "Media type must be image, video or document." });
    if (!mimetype)
      return res.status(400).json({ ok: false, message: "Media MIME type is required." });
    ...
    if (estimatedBytes > MAX_MEDIA_BYTES)
      return res
        .status(413)
        .json({ ok: false, message: "Media is too large. Maximum size is 8 MB in Phase 3." });
    return resultResponse(
      res,
      await runCampaign(recipients, campaignDelay(req.body), (number, recipient) =>
        sendMedia(
          ...
          { caption: personalize(caption, recipient), delayMs: 0 },
        ),
      ),
    );
  } catch (error) {
...
campaignsRouter.post("/buttons", async (req, res, next) => {
  try {
    ...
    if (!String(payload.title || "").trim())
      return res.status(400).json({ ok: false, message: "Button title is required." });
    if (!buttons.length || buttons.length > MAX_BUTTONS)
      return res
        .status(400)
        .json({ ok: false, message: `Buttons require 1–${MAX_BUTTONS} items.` });
    const ids = buttons.map((button) => String(button.id || "").trim());
    const labels = buttons.map((button) => String(button.displayText || "").trim());
    if (ids.some((id) => !id) || labels.some((label) => !label))
      return res
        .status(400)
        .json({ ok: false, message: "Every button needs an ID and display text." });
    if (new Set(ids).size !== ids.length || new Set(labels).size !== labels.length)
      return res.status(400).json({ ok: false, message: "Button IDs and labels must be unique." });
    return resultResponse(
      res,
      await runCampaign(recipients, campaignDelay(req.body), (number, recipient) =>
        sendButtons(instance, number, {
          ...
          buttons: buttons.map((button) => ({
            ...
            phoneNumber: button.phoneNumber
              ? personalize(button.phoneNumber, recipient)
              : undefined,
          })),
        }),
      ),
...
campaignsRouter.post("/list", async (req, res, next) => {
  try {
    ...
    validateRecipients(recipients);
    if (!String(payload.title || "").trim())
      return res.status(400).json({ ok: false, message: "List title is required." });
    if (!String(payload.buttonText || "").trim())
      return res.status(400).json({ ok: false, message: "List button text is required." });
    if (!sections.length)
      return res.status(400).json({ ok: false, message: "At least one list section is required." });
    const rowCount = sections.reduce(
      (total, section) => total + (Array.isArray(section.rows) ? section.rows.length : 0),
      0,
    );
    if (rowCount < 1 || rowCount > MAX_LIST_ROWS)
      return res
        .status(400)
        .json({ ok: false, message: `Lists support 1–${MAX_LIST_ROWS} rows in this phase.` });
    return resultResponse(
      res,
      await runCampaign(recipients, campaignDelay(req.body), (number, recipient) =>
        sendList(instance, number, {
          title: personalize(payload.title, recipient),
          description: personalize(payload.description || "", recipient),
